...
const seedDatabase = async (force = false) => {
  try {
    ...
    if (fs.existsSync(clientReviewsPath)) {
      console.log('Seeding from client generatedReviews.js...');
      ...
      console.log('Database seeded successfully from generatedReviews.js!');
    } else {
      // Fallback flow: load from bundled db.json (which is already structured!)
      const dbJsonPath = path.join(__dirname, '..', 'data', 'db.json');
      if (fs.existsSync(dbJsonPath)) {
        console.log('Loading seed data from bundled db.json fallback...');
        ...
        for (const author of (dbData.authors || [])) {
          await authorsRef.doc(author.id).update(author).catch(() => authorsRef.doc(author.id).set(author));
        }
        for (const product of (dbData.products || [])) {
          await productsRef.doc(product.id).update(product).catch(() => productsRef.doc(product.id).set(product));
        }
        for (const post of (dbData.posts || [])) {
          await postsRef.doc(post.id).update(post).catch(() => postsRef.doc(post.id).set(post));
        }
        console.log('Database seeded successfully from bundled db.json!');
      } else {
        console.error('Neither generatedReviews.js nor db.json was found. Seeding aborted.');
        return;
      }
    }
  } catch (error) {
